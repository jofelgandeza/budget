...
router.get('/viewBranchTargetMon/:id', authUser, authRole(ROLE.BM), async (req, res) => {
    ...
    const vwBranchCode = req.params.id
    const _user = req.user
    // let foundCenterDet = []
    console.log(vwBranchCode)
    ...
    try {
                ...
                const foundCenters = await Center.find({branch: vwBranchCode}, function(err, foundCenters) {
                ...
                //  Pre-determine if items is already existed or saved in Budg_exec_sum Collection
                    const poBudgExecTotLonAmt = await Budg_exec_sum.findOne({branch: vwBranchCode, view_code: "TotLoanAmt"}, function (err, fndTotLonAmt) {
                        fndUnitBudgExecTotLonAmt = fndTotLonAmt
                    })
        
                    const poBudgExecTotIncAmt = await Budg_exec_sum.findOne({branch: vwBranchCode, view_code: "TotProjInc"}, function (err, fndTotIncAmt) {
                        fndUnitBudgExecTotInc = fndTotIncAmt
                    })
        
                    const fndUnitBuExTotProcFeeAmt = await Budg_exec_sum.findOne({branch: vwBranchCode, view_code: "TotProcFee"}, function (err, fndTotProcFeeAmt) {
                        fndUnitBuExTotProcFees = fndTotProcFeeAmt
                    })
                    console.log(poBudgExecTotLonAmt)
        
                    const foundCenterDet = await Center_budget_det.find({branch: vwBranchCode})
                ...
                const newClientCntView = await Center_budget_det.find({branch: vwBranchCode, view_code: "NewLoanClient", client_count_included: true }, function (err, fndNewCliCnt) {
                ...
                const oldClientCntView = await Center_budget_det.find({branch: vwBranchCode, view_code: "OldLoanClient", client_count_included: true}, function (err, fndOldCliCnt) {
                ...
                const resClientCntView = await Center_budget_det.find({branch: vwBranchCode, view_code: "ResClientCount", client_count_included: true}, function (err, fndResCliCnt) {
            ...
            // }
                ...
                poSumView.push({title: "NUMBER OF LOANS", sortkey: 8, group: 1})
        
                const newLoanClientView = await Center_budget_det.find({branch: vwBranchCode, view_code: "NewLoanClient"}, function (err, fndNewCli) {
                ...
                const oldLoanClientView = await Center_budget_det.find({branch: vwBranchCode, view_code: "OldLoanClient"}, function (err, fndOldCli) {
                    ...
                    if (doneReadNLC && doneReadOLC) {
                        poSumView.push({title: "TOTAL NO. OF LOAN", sortkey: 11, group: 1, jan_value : jan_totNoOfLoan, feb_value : feb_totNoOfLoan, mar_value : mar_totNoOfLoan, 
                            ...
                            sep_value : sep_totNoOfLoan, oct_value : oct_totNoOfLoan, nov_value : nov_totNoOfLoan, dec_value : dec_totNoOfLoan
                       }) 
                    }
        
                poSumView.push({title: "AMOUNT OF LOANS", sortkey: 12, group: 2})
        
        
                const newLoanAmtView = await Center_budget_det.find({branch: vwBranchCode, view_code: "NewLoanAmt"}, function (err, fndNewAmt) {
                ...
                const oldLoanAmtView = await Center_budget_det.find({branch: vwBranchCode, view_code: "OldLoanAmt"}, function (err, fndOldAmt) {
                ...
                if (doneReadNLA && doneReadOLA) {
                    ...
                    doneReadForCBU = true
                            ...
                            poSumView.push({title: "CAPITAL BUILD-UP", sortkey: 19, group: 2})
                        ...
                        console.log(err)
                        res.redirect('/branches/'+ vwBranchCode)
                    }
                })
