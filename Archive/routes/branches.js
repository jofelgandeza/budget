...
router.get('/getEmpEditPass/:id/edit', authUser, authRole(ROLE.BM), async (req, res) => {
   ...
   try {
        let brnCod
        const fndEmploye = await Employee.findOne({emp_code: empCode}) //, function (err, foundEmp) {
//            console.log(foundlist)
        if (!isNull(fndEmploye)) {
            foundEmploy = fndEmploye
            brnCod = fndEmploye.branch
            possit = _.trim(fndEmploye.position_code)
           console.log(possit)
           ass_Code = fndEmploye.assign_code
        }
